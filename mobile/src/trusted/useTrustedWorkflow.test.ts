...
describe('useTrustedWorkflow creative drawing intake', () => {
  ...
  test('promotes only the selected candidate with its reviewed draft', async () => {
    const creativeProject: ProjectDetail = {
      ...project,
      id: 'project_creative',
      root_id: 'project_creative',
    ...
    const promotedProject: ProjectDetail = {
      ...creativeProject,
      design_id: 'design_promoted',
    ...
    const promoteCreativeCandidate = jest.fn(async () => ({
      data: promotedProject,
      error: null,
      status: 200,
    }));
    const api = {
    ...
    await act(async () => {
      await result.current.createFromDrawing({
        image_base64: 'c291cmNl',
        instruction: 'Preserve the exact source.',
        title: 'Study',
      });
    });
    const confirmedSpec: JsonObject = {
      jewelry_type: 'ring',
      template: 'solitaire_prong',
    };
    await act(async () => {
      await result.current.promoteCreativeCandidate(
        'candidate_selected',
        confirmedSpec,
      );
    });

    expect(promoteCreativeCandidate).toHaveBeenCalledWith(
      'project_creative',
      'candidate_selected',
      { confirmed_spec: confirmedSpec, created_by: 'usr_designer' },
    );
    expect(result.current.state.project?.design_id).toBe('design_promoted');
    expect(result.current.state.project?.active_design_version).toBe(1);
  });
});
