...
const creativeProjectWithBoardedRegion = (): ProjectDetail => {
  const project = creativeProjectWithSource(1, { region: true });
  const region = project.assets.find(
    (asset) => asset.capability === 'CREATIVE_SOURCE_REGION',
  )!;
  const board: AssetSummary = {
    ...region,
    asset_id: 'reference_board_1',
    parent_asset_id: region.asset_id,
    capability: 'CREATIVE_REFERENCE_BOARD',
    provenance: 'role_labeled_reference_board',
    instruction: 'Role-labeled advisory reference board',
    image_url: 'https://facetta.test/reference-board.png',
  };
  const candidates = (project.creative_candidates ?? []).map((item) => ({
    ...item,
    parent_asset_id: board.asset_id,
  }));
  return {
    ...project,
    creative_candidates: candidates,
    assets: [...project.assets.slice(0, -1), board, ...candidates],
  };
};
...
test('walks through an advisory board to compare the exact geometry crop', async () => {
  const project = creativeProjectWithBoardedRegion();
  await renderCreate(<StudioCreateWorkspace
    gateway={{
      createFromPrompt: jest.fn(), createFromDrawing: jest.fn(),
      completeCreativeDirectionReview: jest.fn(),
  ...
  const reviewSource = screen.getByLabelText('Create starting source');
  expect(reviewSource.props.source.uri).toBe('https://facetta.test/source-region.png');
  expect(reviewSource.props.source.uri).not.toBe('https://facetta.test/reference-board.png');
});

test('fails closed when a source-backed candidate has broken ancestry', async () => {
  const project = creativeProjectWithSource(1);
  const broken = {
    ...project,
    creative_candidates: (project.creative_candidates ?? []).map((item) => ({
      ...item,
      parent_asset_id: 'missing_source_asset',
    })),
  };
  await renderCreate(<StudioCreateWorkspace
    gateway={{
    ...
    owner="designer_1"
    resumeProject={broken}
    onSave={jest.fn()}
  />);

  await loadDirection(1);
  expect(screen.getByText(/starting source lineage is unavailable/i)).toBeTruthy();
  expect(screen.getByText('Continue with Direction 1').parent?.props.accessibilityState.disabled)
    .toBe(true);
});
...
test('keeps only explicitly chosen useful directions as variations', async () => {
  ...
  expect(screen.getByText('How many design options?')).toBeTruthy();
  expect(screen.getByText(
    'Possible contrast: a restrained reading and a more sculptural reading of the same brief.',
  )).toBeTruthy();
  expect(screen.queryByText('Metal color')).toBeNull();
  ...
  expect(screen.getByText(/full generated set stays preserved in this Activity review/i)).toBeTruthy();
  ...
  expect(screen.getByLabelText('Keep Direction 2 as variation').props.accessibilityState)
  ...
  expect(screen.getByLabelText('Direction 1').props.accessibilityHint)
    .toBe('Preserved in Activity review');
  expect(screen.getByLabelText('Keep Direction 1 as variation').props.accessibilityState)
    .toEqual({ checked: false, disabled: false });
  expect(within(screen.getByLabelText('Direction 3')).getByText('Selected to refine'))
    .toBeTruthy();
  await loadDirection(3);
  await fireEvent.press(screen.getByText('Continue with Direction 3 · keep 2 variations'));
  await waitFor(() => expect(completeCreativeDirectionReview).toHaveBeenCalledTimes(1));
...
test('uses one-tap visual guidance without opening the image picker', async () => {
  const createFromPrompt = jest.fn(async () => ({
  ...
  await renderCreate(<StudioCreateWorkspace
    ...
    owner="designer_1"
    initialSentence="A quiet sculptural signet ring."
  ...
  expect(screen.getByLabelText(
    '2 design options: Two distinct approaches · Recommended. Possible contrast: a restrained reading and a more sculptural reading of the same brief.',
  )).toBeTruthy();
  await fireEvent.press(screen.getByLabelText('Guide the look'));
  await fireEvent.press(screen.getByLabelText('Metal color: Yellow gold'));
  await fireEvent.press(screen.getByLabelText('Color accents: Green'));
  await fireEvent.press(screen.getByLabelText('Surface & finish: Satin / brushed'));
  await fireEvent.press(screen.getByLabelText('Visual mood: Minimal'));

  expect(onRequestReference).not.toHaveBeenCalled();
  expect(screen.getByText('Yellow gold · Green · Satin / brushed · Minimal')).toBeTruthy();
  expect(screen.queryByText('Construction detail')).toBeNull();
  expect(screen.getByLabelText('Add inspiration images').props.accessibilityState).toEqual({
    expanded: false,
  });
  await fireEvent.press(screen.getByLabelText('Add inspiration images'));
  expect(onRequestReference).not.toHaveBeenCalled();
  expect(screen.getByText('Specific jewelry detail image')).toBeTruthy();
  expect(screen.getByText(/engineering is reviewed later/i)).toBeTruthy();
  expect(screen.getByText('Visual mood image')).toBeTruthy();
  expect(screen.getByText(/not logos or another jewelry design/i)).toBeTruthy();

  await fireEvent.press(screen.getByText('Create 2 design options'));
  await waitFor(() => expect(createFromPrompt).toHaveBeenCalledWith({
    prompt: 'A quiet sculptural signet ring.',
    creative_intent: {
      metal_color: 'yellow',
      color_accent: 'green',
    ...
    variation_count: 2,
    owner: 'designer_1',
    title: 'A quiet sculptural signet ring.',
  }));
  expect(onRequestReference).not.toHaveBeenCalled();
});
...
test('defaults to no retained variations without adding global retention modes', async () => {
  ...
  await loadDirection(2);
  await loadDirection(3);
  expect(screen.getByLabelText('Keep Direction 2 as variation')).toBeTruthy();
...
test('caps explicit retained variations at three without hiding the review set', async () => {
  const completeCreativeDirectionReview = jest.fn(async ({ selectedCandidateId }) => ({
    data: {
      project: {
        ...creativeProject(6),
  ...
  await fireEvent.press(screen.getByLabelText('Keep Direction 3 as variation'));
  await fireEvent.press(screen.getByLabelText('Keep Direction 4 as variation'));
  expect(within(screen.getByLabelText('Direction 5')).getByText('Preserved in Activity review'))
    .toBeTruthy();
  expect(within(screen.getByLabelText('Direction 6')).getByText('Preserved in Activity review'))
    .toBeTruthy();
  ...
  expect(screen.getByLabelText('Direction 5').props.accessibilityHint)
    .toBe('Preserved in Activity review');
  expect(screen.getByLabelText('Keep Direction 5 as variation').props.accessibilityState)
    .toEqual({ checked: false, disabled: true });
  await fireEvent.press(screen.getByText('Continue with Direction 1 · keep 3 variations'));
...
test('a deferred generation clears only its exact submitted draft', async () => {
  ...
  await act(() => {
    createCompletion = fireEvent.press(screen.getByText('Create 2 design options'));
  });
...
test('reopens a durable reviewing Create job without inferring retained variations', async () => {
...
test('starting another brief clears the previous review error and collapses advanced setup', async () => {
  ...
  await fireEvent.press(screen.getByLabelText('Guide the look'));
  expect(screen.getByText('Metal color')).toBeTruthy();
  await fireEvent.press(screen.getByText('Create 2 design options'));
  await loadDirection(1);
  await fireEvent.press(screen.getByText('Continue with Direction 1'));
  ...
  expect(screen.queryByText('Facetta could not connect. Check your connection and try again.'))
    .toBeNull();
  expect(screen.queryByText('Metal color')).toBeNull();
  expect(screen.getByLabelText('Guide the look').props.accessibilityState)
...
test('keeps an already-created direction set when the designer starts another brief', async () => {
  ...
  await fireEvent.press(screen.getByText('Create 2 design options'));
  expect(await screen.findByText(/full generated set stays preserved in this Activity review/i)).toBeTruthy();
  await loadDirection(1);
  expect(screen.getByText('Continue with Direction 1')).toBeTruthy();
  await fireEvent.press(screen.getByText('Leave in Activity & start another'));
...
test('sends every enabled role with the master geometry input', async () => {
  ...
  await fireEvent.press(screen.getByText('Upload drawing, photo, or render'));
  expect(screen.getByText('What did you upload?')).toBeTruthy();
  await fireEvent.press(screen.getByText('Drawing'));
  await fireEvent.press(screen.getByLabelText('Guide the look'));
  await fireEvent.press(screen.getByLabelText('Surface & finish: Satin / brushed'));
  await fireEvent.press(screen.getByLabelText('Add inspiration images'));
  await fireEvent.press(screen.getByLabelText('Choose material or finish image'));
  await fireEvent.changeText(screen.getByLabelText('Design sentence'), 'Preserve the silhouette and make it feel lighter.');

  expect(screen.getByText('Create 2 design options').parent?.props.accessibilityState).toEqual({ disabled: true });
  ...
  expect(screen.getByLabelText('Material & finish image preview').props.source.uri).toBe(
  ...
  await fireEvent.press(screen.getByLabelText('Inspect Material & finish image · Brushed gold reference in detail'));
  expect(screen.getByText('Material & finish image · Brushed gold reference')).toBeTruthy();
  await fireEvent.press(screen.getByLabelText('Close image inspector'));
  expect(createProjectFromDrawing).not.toHaveBeenCalled();

  await loadReferencePreview('Visual source preview');
  await loadReferencePreview('Material & finish image preview');
  expect(screen.getByText('Create 2 design options').parent?.props.accessibilityState).toEqual({ disabled: false });
  await fireEvent.press(screen.getByText('Create 2 design options'));
...
test('accepts role-labeled advisory guidance after a sentence without requiring a master', async () => {
  ...
  await fireEvent.press(screen.getByLabelText('Guide the look'));
  await fireEvent.press(screen.getByLabelText('Add inspiration images'));
  const addMaterial = screen.getByLabelText('Choose material or finish image');
  ...
  expect(screen.getByText('Facetta decides the look')).toBeTruthy();

  await fireEvent.changeText(
    screen.getByLabelText('Design sentence'),
    'A broad sculptural gold cuff with one clean opening.',
  );
  expect(screen.getByLabelText('Choose material or finish image').props.accessibilityState).toEqual({
    disabled: false,
  });
  await fireEvent.press(screen.getByLabelText('Choose material or finish image'));
  expect(onRequestReference).toHaveBeenCalledWith('material_style');
  expect(screen.getByText('Facetta decides the look · 1 inspiration image')).toBeTruthy();
...
test('rejects an advisory-only setup until the designer supplies a sentence or master', async () => {
  ...
  expect(screen.queryByText('Master geometry required')).toBeNull();
  expect(screen.getByText('Create 2 design options').parent?.props.accessibilityState).toEqual({
    disabled: true,
  });
  await fireEvent.press(screen.getByText('Create 2 design options'));
  expect(createFromPrompt).not.toHaveBeenCalled();
...
test('the latest deferred picker merges into the current controlled draft', async () => {
  ...
  await act(() => {
    firstCompletion = fireEvent.press(screen.getByText('Upload drawing, photo, or render'));
  });
  await act(() => {
    secondCompletion = fireEvent.press(screen.getByText('Upload drawing, photo, or render'));
  });
...
test('a picker resolving after unmount cannot write to its former controlled owner', async () => {
  ...
  await act(() => {
    pickerCompletion = fireEvent.press(screen.getByText('Upload drawing, photo, or render'));
  });
  await view.unmount();
  await act(async () => {
    pending.resolve({
...
test('hydrates a controlled draft with its master source truth and output count', async () => {
  ...
  await renderCreate(<StudioCreateWorkspace
    ...
    draft={{
      ...
      guidance: EMPTY_STUDIO_CREATE_DRAFT.guidance,
      candidateCount: 4,
    }}
