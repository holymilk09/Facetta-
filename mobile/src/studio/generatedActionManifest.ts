/* Generated by scripts/generate_studio_action_manifest.py. */
/* Do not edit this file directly. */
import { StudioActionFieldDefinition, StudioAuthority, StudioExecutionMode, StudioLane, StudioOutputType, StudioReviewAuthority } from './contracts';

export interface StudioJobActionManifestEntry {
  lane: StudioLane;
  execution_mode: StudioExecutionMode;
  review_authority: StudioReviewAuthority;
  credits_per_output: number;
  min_requested_outputs: number;
  max_requested_outputs: number;
  input_requirements: readonly string[];
  context_requirements: readonly string[];
  output_type: StudioOutputType;
  authority: StudioAuthority;
  ui_schema: readonly StudioActionFieldDefinition[];
}

export const STUDIO_JOB_ACTION_MANIFEST = {
  "create": {
    "lane": "fast_visual",
    "execution_mode": "candidate_job",
    "review_authority": "candidate_decision",
    "credits_per_output": 15,
    "min_requested_outputs": 1,
    "max_requested_outputs": 4,
    "input_requirements": [
      "brief_or_reference"
    ],
    "context_requirements": [],
    "output_type": "design_revision",
    "authority": "design_record",
    "ui_schema": [
      {
        "id": "brief",
        "label": "Design brief",
        "kind": "text",
        "required": false
      },
      {
        "id": "master",
        "label": "Optional drawing",
        "kind": "reference",
        "required": false,
        "referenceRole": "master_geometry"
      }
    ]
  },
  "vary": {
    "lane": "instant",
    "execution_mode": "instant_transaction",
    "review_authority": "none",
    "credits_per_output": 0,
    "min_requested_outputs": 0,
    "max_requested_outputs": 0,
    "input_requirements": [
      "direction"
    ],
    "context_requirements": [
      "active_project",
      "active_revision"
    ],
    "output_type": "variation_set",
    "authority": "design_record",
    "ui_schema": [
      {
        "id": "direction",
        "label": "Variation name",
        "kind": "text",
        "required": true
      }
    ]
  },
  "refine": {
    "lane": "trusted_structural",
    "execution_mode": "candidate_job",
    "review_authority": "candidate_decision",
    "credits_per_output": 20,
    "min_requested_outputs": 1,
    "max_requested_outputs": 1,
    "input_requirements": [
      "instruction"
    ],
    "context_requirements": [
      "active_project",
      "active_revision"
    ],
    "output_type": "design_revision",
    "authority": "design_record",
    "ui_schema": [
      {
        "id": "instruction",
        "label": "Change instruction",
        "kind": "text",
        "required": true
      },
      {
        "id": "mask",
        "label": "Target region",
        "kind": "reference",
        "required": false,
        "referenceRole": "edit_mask"
      }
    ]
  },
  "views": {
    "lane": "fast_visual",
    "execution_mode": "candidate_job",
    "review_authority": "candidate_decision",
    "credits_per_output": 15,
    "min_requested_outputs": 1,
    "max_requested_outputs": 1,
    "input_requirements": [
      "view_set"
    ],
    "context_requirements": [
      "active_project",
      "active_revision",
      "exact_specification"
    ],
    "output_type": "view_set",
    "authority": "visual_preview",
    "ui_schema": [
      {
        "id": "view_set",
        "label": "Views",
        "kind": "select",
        "required": true
      }
    ]
  },
  "angles": {
    "lane": "fast_visual",
    "execution_mode": "candidate_job",
    "review_authority": "candidate_decision",
    "credits_per_output": 18,
    "min_requested_outputs": 3,
    "max_requested_outputs": 3,
    "input_requirements": [
      "angle_set"
    ],
    "context_requirements": [
      "active_project",
      "active_revision",
      "selected_pre_spec_visual"
    ],
    "output_type": "visual_angle_set",
    "authority": "visual_preview",
    "ui_schema": [
      {
        "id": "angle_set",
        "label": "Front, three-quarter, and side",
        "kind": "fixed",
        "required": true
      }
    ]
  },
  "present": {
    "lane": "fast_visual",
    "execution_mode": "candidate_job",
    "review_authority": "candidate_decision",
    "credits_per_output": 18,
    "min_requested_outputs": 1,
    "max_requested_outputs": 4,
    "input_requirements": [
      "destination"
    ],
    "context_requirements": [
      "active_project",
      "active_revision"
    ],
    "output_type": "presentation_pack",
    "authority": "visual_preview",
    "ui_schema": [
      {
        "id": "destination",
        "label": "Presentation destination",
        "kind": "select",
        "required": true
      },
      {
        "id": "direction",
        "label": "Optional art direction",
        "kind": "text",
        "required": false
      }
    ]
  },
  "factory": {
    "lane": "trusted_structural",
    "execution_mode": "terminal_job",
    "review_authority": "backend_transaction",
    "credits_per_output": 28,
    "min_requested_outputs": 1,
    "max_requested_outputs": 1,
    "input_requirements": [],
    "context_requirements": [
      "active_project",
      "active_revision",
      "exact_specification",
      "factory_eligible"
    ],
    "output_type": "factory_review_pack",
    "authority": "production_review",
    "ui_schema": []
  }
} as const satisfies Record<string, StudioJobActionManifestEntry>;
