...
def accept_studio_view_candidate(
    ...
    *,
    ...
    if candidate_affinity is None:
        raise StudioViewError(
            "view_candidate_provider_unproven",
            "the View provider could not be proven from generation evidence",
            status_code=422,
        )
    if source_affinity is not None and (
        ...
        raise StudioViewError(
            "view_candidate_provider_mismatch",
            "the View was generated by a different image provider than its "
            "source revision",
            status_code=409,
        )
    asset = ImageAsset(
    ...
    attach_asset_provider_affinity(
        ...
        derivation="accepted_view_attempt",
    )
