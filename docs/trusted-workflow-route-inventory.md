...
| `GET /projects/{project_id}/creative-directions/review-draft` | Canonical | Studio Create Activity resume; Trusted client; `test_creative_project_api.py` | Returns only the authenticated owner's mutable, versioned direction choice for an exact reviewing Create job. The draft is deliberately separate from immutable design history; missing, foreign, committed, stale-job, or candidate-drift state fails closed. |
| `PUT /projects/{project_id}/creative-directions/review-draft` | Canonical | Studio Create candidate chooser; Trusted client; `test_creative_project_api.py` | Optimistically creates or replaces the complete selected/retained review intent against an exact project, reviewing Create job, owner, candidate set, and expected version. It creates no revision and no charge; the atomic commit remains the only immutable acceptance authority. |
...
| `POST /studio/projects/{project_root_id}/revisions/{asset_id}/variations` | Canonical | Studio historical branching; `test_studio_historical_variation.py` | Creates an independent sibling Variation from any exact immutable revision exposed by Studio history without restoring it first. The request separately binds the selected source asset/version/hash and current active compare-and-set guard; the atomic response proves family, branch, byte/spec, and component-map lineage while leaving the source Variation's canonical history unchanged. |
| `POST /studio/projects/{project_root_id}/creative-candidates/{candidate_id}/variations` | Canonical | Studio Create comparison and generated-direction archive; pre-spec journey tests | Saves an explicitly kept, unselected Create candidate as a named sibling Variation after the Original direction is fixed, including after the active Original later gains specification authority. A required operation ID makes transport retries idempotent. Candidate alternatives remain durable review records, never masquerade as sequential revisions, and the exact raster/component map is copied without inventing authority. |
...
| `POST /assets/{asset_id}/markup/interpretations/{interpretation_id}/confirm` | Canonical | Studio Refine gateway; markup confirmation authority tests | Retain; confirms one immutable owner-, source-, markup-, specification-, and component-map-bound interpretation before marked generation. |
