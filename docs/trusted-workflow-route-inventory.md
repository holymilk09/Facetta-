...
| `POST /projects/{project_id}/creative-candidates/{candidate_id}/select` | Deprecated compatibility | Trusted compatibility client; historical development and evaluation tests | Retained outside production so historical projects and evaluation fixtures remain readable. It can select without the immutable decision/revision provenance required by Studio, so active callers must use `POST /projects/{project_id}/creative-directions/commit`. |
...
| `PUT /studio/design-families/{family_id}/favorite` | Canonical | Studio Collections workspace; `test_workspace_collections_api.py` | Idempotently favorites one owner-scoped Design Family without changing its design-activity timestamp, lineage, jobs, or credits. |
| `DELETE /studio/design-families/{family_id}/favorite` | Canonical | Studio Collections workspace; `test_workspace_collections_api.py` | Idempotently removes family-level favorite metadata without changing its design-activity timestamp, lineage, jobs, or credits. |
| `PUT /studio/design-families/{family_id}/tags` | Canonical | Studio Collections workspace; `test_workspace_collections_api.py` | Replaces normalized owner-scoped family tags without changing variation metadata, design activity, lineage, jobs, or credits. |
| `GET /studio/collections` | Canonical | Studio Collections workspace; `test_workspace_collections_api.py` | Lists or searches active owner-scoped organizational Collections without reading across tenants or flattening family lineage. |
| `GET /studio/collection-memberships` | Canonical | Studio Collections workspace; `test_workspace_collections_api.py` | Returns active visible Collection IDs grouped by owner-scoped Design Family in one query, excluding removed memberships and archived or deleted Collections. |
| `POST /studio/collections` | Canonical | Studio Collections workspace; `test_workspace_collections_api.py` | Creates an optional generic or template-guided Collection outside canonical family, variation, and revision history. |
| `PATCH /studio/collections/{collection_id}` | Canonical | Studio Collections workspace; `test_workspace_collections_api.py` | Renames, updates metadata, or archives an owner-scoped Collection; archive removes only active organizational memberships. |
| `DELETE /studio/collections/{collection_id}` | Canonical | Studio Collections workspace; `test_workspace_collections_api.py` | Soft-deletes a Collection and removes only its active memberships, preserving every family, variation, asset, and immutable revision. |
| `GET /studio/design-families/{family_id}/collections` | Canonical | Studio Collections workspace; `test_workspace_collections_api.py` | Lists the active Collections currently organizing one owner-scoped Design Family. |
| `PUT /studio/design-families/{family_id}/collections/{collection_id}` | Canonical | Studio Collections workspace; `test_workspace_collections_api.py` | Idempotently adds one owner-scoped family to one active Collection without duplicating assets or history. |
| `DELETE /studio/design-families/{family_id}/collections/{collection_id}` | Canonical | Studio Collections workspace; `test_workspace_collections_api.py` | Idempotently removes a family membership while retaining a tombstone that prevents legacy migration from resurrecting it. |
...
| `GET /studio/projects/{project_root_id}/continuation-prompts` | Canonical | Studio Refine prompt history; Trusted client; `test_studio_visual_preview_api.py` | Returns the chronological, immutable designer-authored Refine prompts and their derived candidate/job outcomes. Hidden provider preservation instructions are never exposed as user history. |
| `GET /studio/projects/{project_root_id}/preview-candidates` | Canonical | Unified Studio Refine resume; `test_studio_visual_preview_api.py`, `test_catalog_revision_api.py`, `test_studio_markup_candidate_durability.py` | Lists owner-scoped temporary visual, catalog-revision, and mark-up candidates through one discriminated Studio projection. Each candidate retains its authoritative project, source asset, optional exact design version, image-run, and Studio-job lineage. |
| `GET /studio/preview-candidates/{candidate_id}` | Canonical | Unified Studio Refine review; `test_studio_visual_preview_api.py`, `test_catalog_revision_api.py`, `test_studio_markup_candidate_durability.py` | Returns one owner-scoped normalized candidate without replacing its kind-specific authority. Foreign, malformed, and missing candidates remain non-enumerating. |
| `GET /studio/preview-candidates/{candidate_id}/image` | Canonical | Unified Studio Refine review; `test_studio_visual_preview_api.py`, `test_catalog_revision_api.py`, `test_studio_markup_candidate_durability.py` | Returns private, no-store bytes for one reviewable normalized candidate after delegating availability and integrity checks to its authoritative visual, catalog, or mark-up service. |
| `POST /studio/preview-candidates/{candidate_id}/decision` | Canonical | Unified Studio Refine decision; `test_studio_visual_preview_api.py`, `test_catalog_revision_api.py`, `test_studio_markup_candidate_durability.py` | Dispatches Apply, Save as Variation, or Discard to the candidate kind's existing atomic transaction. Exact source/version compare-and-set, immutable pre-acceptance history, idempotent terminal replay, provenance, and Studio-job accounting remain enforced by the canonical service. |
...
| `GET /projects/{project_id}/factory-pack.zip` | Canonical | Trusted client; `test_trusted_handoff.py`; corrected designer E2E; ten-template DXF parity suite | Retain; exact approved archive. Only validated specification JSON and confirmed fact schedules are authoritative records. Deterministic SVG is a schematic dimensional diagram, and transform-aware DXF remains a bundled non-authoritative R12 exchange reference because neither establishes solid, tolerance-bearing fabrication geometry. An exact-version confirmed line drawing is included only as manifest-labeled, non-authoritative discussion context. |
...
| `GET /image-runs/{run_id}/candidates/{candidate_id}/image` | Deprecated compatibility | Hidden trusted-workflow client; historical markup tests | Retain outside production while compatibility readers migrate. Studio uses the durable owner-scoped `/studio/markup-candidates/{run_id}/{candidate_id}/image` route. |
| `POST /image-runs/{run_id}/candidates/{candidate_id}/accept` | Deprecated compatibility | Hidden trusted-workflow client; historical markup tests | Retain outside production for legacy in-memory candidates. Studio promotes durable candidates through the exact-lineage `/studio/markup-candidates/{run_id}/{candidate_id}/accept` route. |
| `POST /image-runs/{run_id}/candidates/{candidate_id}/discard` | Deprecated compatibility | Hidden trusted-workflow client; historical markup tests | Retain outside production for legacy in-memory candidates. Studio terminally resolves durable candidates through the exact-lineage `/studio/markup-candidates/{run_id}/{candidate_id}/discard` route. |
...
| `POST /assets/{asset_id}/markup/apply` | Canonical | Studio Refine gateway; Trusted client; markup tests | Production accepts only one exact-version instruction bound to a durable Refine job and returns a temporary candidate; owner-scoped Studio candidate routes own Apply, Save as Variation, and Discard. Deprecated no-version or unlinked compatibility remains non-production-only until historical callers migrate. |
