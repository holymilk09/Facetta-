...
| `POST /studio/projects/{project_id}/visual-angle-sets` | Canonical | Studio selected-direction angle set; `test_studio_visual_angle_set_api.py` | Generates exactly front, three-quarter, and side visual camera studies from one selected pre-spec direction. All three remain temporary and review-only, with exact source hashes plus per-output image-run, routing, and QA evidence; a hard failure terminates the whole uncharged job without saving partial assets. |
| `GET /studio/visual-angle-sets/by-job/{studio_job_id}` | Canonical | Studio Angles resume from Activity; `test_studio_visual_angle_set_api.py` | Resolves one owner-scoped canonical `angles` job to its durable review set so app restart or navigation never depends on an in-memory candidate map. Missing and foreign job bindings fail closed without disclosing existence. |
| `GET /studio/visual-angle-sets/{angle_set_id}` | Canonical | Studio selected-direction angle set resume; `test_studio_visual_angle_set_api.py` | Revalidates owner, selected source, source bytes, job, image runs, output hashes, routing, and forced-review QA before returning one unresolved three-angle decision group. Stale or corrupted evidence expires fail-closed. |
| `GET /studio/visual-angle-sets/{angle_set_id}/candidates/{candidate_id}/image` | Canonical | Studio selected-direction angle review; `test_studio_visual_angle_set_api.py` | Returns private, no-store bytes for one temporary visual angle only while its complete source-bound set remains reviewable. |
| `POST /studio/visual-angle-sets/{angle_set_id}/accept` | Canonical | Studio selected-direction angle review; `test_studio_visual_angle_set_api.py` | Atomically accepts all three reviewed outputs as derived `ANGLE_VIEW` assets and charges exactly three accepted outputs. It never advances or mutates the active revision and creates no specification or Factory authority. |
| `POST /studio/visual-angle-sets/{angle_set_id}/discard` | Canonical | Studio selected-direction angle review; `test_studio_visual_angle_set_api.py` | Atomically rejects all three image runs, clears temporary bytes, cancels the linked job with zero completed or charged outputs, and creates no assets. |
...
| `GET /projects/{project_id}/factory-pack/files/{artifact_name}` | Canonical | Studio Factory workspace; Trusted client; `test_trusted_handoff.py`; `test_auth_boundary.py` | Returns one authenticated, manifest-allowlisted review-pack artifact with its correct media type, exact SHA-256 evidence, and private no-store caching. Nested paths, missing files, and foreign projects fail closed. |
