...
The inventory contains all 128 currently registered HTTP operations. “Tests”
...
| `POST /projects/{root_id}/render` | Canonical | Trusted client; `designer-reference-e2e-2026-07-11`; project render tests | QA-gated beauty render from a confirmed imported reference; rejects incomplete source coverage before provider work and persists only an accepted primary revision/ImageRun atomically. |
| `POST /projects/{root_id}/product-photo` | Canonical | Product-photo compiler/project tests; `designer-product-photo-live-2026-07-11` | QA-gated ecommerce restage. Inherits the exact spec version, rejects physical redesign instructions, and persists clean `PRODUCT_PHOTO` bytes only after pass or explicit warning review. |
