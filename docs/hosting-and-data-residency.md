...
This decision covers database placement only. The current app login is a local
prototype; Supabase Auth, backend JWT verification, ownership enforcement, and
RLS are still required before real user data is hosted. Until then, keep the
Facetta tables out of the Supabase Data API.
